...
RC MvccTrx::update_record(Table *table, Record &record, std::vector<ParsedUpdateItem> &update_items)
{
  // 获取开始和结束字段
  Field begin_field;
  Field end_field;
  ...
  /*while(record.mutex_bool == 1) {
    continue;
  }*/
  std::unique_lock<std::mutex> lock(record.mutex_);
  // 检查记录是否被其他事务更新
  /*int32_t end_xid = end_field.get_int(record);
  if (end_xid != trx_kit_.max_trx_id()) {
    // 记录已被其他事务更新，返回并发冲突错误
    LOG_ERROR("concurrency conflit: other transaction is updating this record. end_xid=%d, current trx id=%d, rid=%s",
              end_xid, trx_id_, record.rid().to_string().c_str());
  ...
  record.mutex_bool++;
  //Record new_record;
  //RC rc = table->get_record(record.rid(), new_record);
  //if(rc != RC::SUCCESS) {
  //  return rc;
  //}

  for (auto &item : update_items) {  //直接修改原纪录，不做插入处理
    int offset = item.field->meta()->offset();
    int len = item.field->meta()->len();
    memcpy(record.data() + offset, item.value.data(), len);
  }
