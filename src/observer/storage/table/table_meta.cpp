...
const IndexMeta *TableMeta::find_index_by_field(const char *field) const    //index field size is 1
{
  for (const IndexMeta &index : indexes_) {
...
const IndexMeta *TableMeta::find_index_by_fields(std::vector<const char *> &fields) const  //index field size greater than 1
{
  for (const IndexMeta &index : indexes_) {
