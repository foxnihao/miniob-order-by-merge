...
RC Table::drop(const char *base_dir)
{
  ...
  rc=sync();
  if (rc != RC::SUCCESS) {
    LOG_ERROR("Failed to sync table %s:%s", base_dir, name());
    return rc;
  }
...
RC Table::insert_record(Record &record)
{
  ...
  rc = insert_entry_of_indexes(record.data(), record.rid());   //尝试插入索引项
...
RC Table::make_record(int value_num, const Value *values, Record &record)
{
  ...
  for (int i = 0; i < value_num; i++) {
    ...
    if (field->type() != value.attr_type()) {
      ...
      else if (field->type() == FLOATS && value.attr_type() == INTS) {
        continue;
      }
      else{
  ...
  for (int i = 0; i < value_num; i++) {
    ...
    if (field->type() == FLOATS && value.attr_type() == INTS) {
      float float_val = (float)values[i].get_int();
...
RC Table::create_index(Trx *trx, const std::vector< const FieldMeta*> &field_metas, const char *index_name, bool is_unique)
{
  ...
  if (rc != RC::SUCCESS) {
    ...
    for(int i = 0; i < field_metas.size(); i++) {
      LOG_INFO("field_name:%s",field_metas[i]->name());
    }
    return rc;
  }
...
RC Table::update_record(Record &record, Field *field, Value *value)
{
  ...
  int offset=field->meta()->offset();
  int len=field->meta()->len();
  memcpy(old_record.data()+offset, value->data(), len);

  rc = insert_record(old_record);  //捏妈妈的  old_record 才是新值
  if (rc != RC::SUCCESS) {
    LOG_ERROR("Failed to insert new record. table name=%s, rc=%s", name(), strrc(rc));
    return rc;
  }

  rc = delete_record(record);
  if (rc != RC::SUCCESS) {
    LOG_ERROR("Failed to delete old record. table name=%s, rc=%s", name(), strrc(rc));
    return rc;
  }
...
RC Table::update_record(Record &record, std::vector<ParsedUpdateItem> &update_items)
{
  RC rc= RC::SUCCESS;
  Record old_record;
  rc = get_record(record.rid(), old_record);
  ...
  for (const auto &item : update_items) {
    int offset = item.field->meta()->offset();
    int len = item.field->meta()->len();
    memcpy(old_record.data() + offset, item.value.data(), len);
  }

  rc = insert_record(old_record);
  if (rc != RC::SUCCESS) {
    LOG_ERROR("Failed to rollback old record. table name=%s, rc=%s", name(), strrc(rc));
    return rc;
  }
...
RC Table::insert_entry_of_indexes(const char *record, const RID &rid)
{
  ...
  for (Index *index : indexes_) {
    ...
    if(!index_meta.is_unique()){
      ...
      //不严谨的bug，如果存在多个索引文件，update值不涉及唯一索引键，但是案例会扫描所有索引，拿到的记录在通过唯一索引检查是会出错，
      std::vector<const char*> keys;
      std::vector<int> lens;
      int offset = 0;
      auto index_field_metas = index->field_meta();
      //auto field_metas = table_meta_.field_metas();
      const std::vector<FieldMeta>* field_metas_ptr = table_meta_.field_metas();  // 调用 field_metas() 函数获取指针
      std::vector<FieldMeta> field_metas(field_metas_ptr->begin(), field_metas_ptr->end());

      int indexMeta_idx = 0;
      int offset_;
      bool has_null = false;
      int off1 = 0;
      for (int i = 0; i < index_field_metas.size(); ++i) {
        bool this_is_null = true;
        for (int j = 0; j<std::min(4,index_field_metas[i].len()); ++j) {
          auto *st = record+off1;
          auto *it = st+j;
          if (*it != '\n') {this_is_null = false; break;}
        }
        if (this_is_null) has_null = true;
        off1 += index_field_metas[i].len();
      }
      while(indexMeta_idx < index_field_metas.size()){
        offset_ = 0;
        for(int it=0;it<table_meta_.field_metas()->size();it++){
          if(strcmp(index_field_metas[indexMeta_idx].name(),field_metas[it].name()) == 0){
            keys.push_back(record+offset_);
            lens.push_back(index->field_meta()[it].len());
            indexMeta_idx++;
            break;
          }
          offset_ += field_metas.at(it).len();
        }
      }
      if (!has_null) {
        IndexScanner *scanner=index->create_scanner(keys,lens,true,keys,lens,true);
        RID rid_;
        rc=scanner->next_entry(&rid_);
        scanner->destroy();
        if(rc==RC::SUCCESS){
          return RC::INTERNAL;
        }
...
bool check_date(int year, int month, int day){
  ...
  if(is_leapyear) days[2] = 29;
  return (year>0) && (month > 0) && (month < 13) && (day > 0) && (day < days[month] + 1 ); 
}
