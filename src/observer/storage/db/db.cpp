...
RC Db::create_table(const char *table_name, int attribute_count, const AttrInfoSqlNode *attributes,
                    const Value *subquery_result, Trx *trx)
{
  RC rc = RC::SUCCESS;
  // check table_name
  ...
  if (subquery_result) {
    if (subquery_result->attr_type() != VAL_LIST) return RC::INTERNAL;
    InsertPhysicalOperator insert_oper = InsertPhysicalOperator(table);
    const std::vector<Value> &val_lists = subquery_result->get_val_list();
    for (const Value &val_list_value : val_lists) {
      std::vector<Value> values = val_list_value.get_val_list();
      insert_oper.set_values(&values);
      rc = insert_oper.open(trx); // 插入记录
      if (rc != RC::SUCCESS) return rc;
      rc = insert_oper.close();
      if (rc != RC::SUCCESS) return rc;
    }
  }
