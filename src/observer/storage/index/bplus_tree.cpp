...
RC BplusTreeHandler::create(const char *file_name,AttrType attr_type,int attr_length,int internal_max_size /* = -1*/ ,int leaf_max_size /* = -1*/){
  return create(file_name, std::vector<AttrType>{attr_type}, std::vector<int>{attr_length}, internal_max_size, leaf_max_size);
}
...
RC BplusTreeHandler::open(const char *file_name)
{
  ...
  for(AttrType it: file_header_.attr_types) {
    attr_types.push_back(it);
  }
  std::vector<int> attr_lengths;
  for(int it: file_header_.attr_lengths) {
    attr_lengths.push_back(it);
  } 
  key_comparator_.init(attr_types, attr_lengths);
  key_printer_.init(attr_types, attr_lengths);
...
MemPoolItem::unique_ptr BplusTreeHandler::make_key(std::vector<const char *>user_key, const RID &rid)
{
  ...
  assert(file_header_.attr_num == user_key.size());
  int offset_ = 0;
  for(int i=0; i<file_header_.attr_num; i++) {
    memcpy(static_cast<char *>(key.get()) + offset_, user_key[i], file_header_.attr_lengths[i]);
    offset_ += file_header_.attr_lengths[i];
  }
  memcpy(static_cast<char *>(key.get()) + offset_, &rid, sizeof(rid));
  return key;
}
...
/*MemPoolItem::unique_ptr BplusTreeHandler::make_key(std::vector<const char *>user_key, const RID &rid)
{
  ...
  assert(file_header_.attr_num == user_key.size());
  for(int i=0; i<file_header_.attr_num; i++) {
    memcpy(static_cast<char *>(key.get()) + key_size(i), user_key[i], file_header_.attr_lengths[i]);
  }
  memcpy(static_cast<char *>(key.get()) + key_size(file_header_.attr_num -1), &rid, sizeof(rid));
  return key;
}

int BplusTreeHandler::key_size(int current) const
{
  int attr_lengths_sum = 0;
  for (int i = 0; i < current; i++) {
    attr_lengths_sum += file_header_.attr_lengths[i];
  }
  return attr_lengths_sum + sizeof(RID);
}*/

RC BplusTreeHandler::insert_entry(const char* user_key, const RID *rid){
  return insert_entry(std::vector<const char *>{user_key}, rid);
}
...
RC BplusTreeHandler::get_entry(const char *user_key, int key_len, std::list<RID> &rids)
{
  ...
  RC rc = scanner.open(std::vector<const char *>{user_key}, std::vector<int>{key_len}, true /*left_inclusive*/, std::vector<const char *>{user_key}, std::vector<int>{key_len}, true /*right_inclusive*/);
  if (rc != RC::SUCCESS) {
...
{
  return delete_entry(std::vector<const char *>{user_key}, rid);
}
...
RC BplusTreeHandler::delete_entry(std::vector<const char *> user_key, const RID *rid)
{
  ...
  for(int i=0; i<user_key.size(); i++) {
    memcpy(static_cast<char *>(key) + offset_, user_key[i], file_header_.attr_lengths[i]);
    offset_ += file_header_.attr_lengths[i];
  }
  memcpy(static_cast<char *>(key) + offset_, rid, sizeof(*rid));
...
{
  return open(std::vector<const char *>{left_user_key}, std::vector<int>{left_len}, left_inclusive, std::vector<const char *>{right_user_key}, std::vector<int>{right_len}, right_inclusive);
}

RC BplusTreeScanner::open(const std::vector<const char *>left_user_key,const std::vector<int> left_len, bool left_inclusive, 
                          const std::vector<const char *>right_user_key,const std::vector<int> right_len, bool right_inclusive)
{
  //     ?要在这里修正indexkeys？但貌似没办法，这个类里边成员没啥用
  RC rc = RC::SUCCESS;
  if (inited_) {
  ...
  inited_ = true;
  first_emitted_ = false;    //first_emitted

  // 校验输入的键值是否是合法范围
  assert(left_user_key.size() == right_user_key.size());

  if(left_user_key.size()>0 && right_user_key.size()> 0 && left_user_key[0] && right_user_key[0]){
    auto &attr_comparators = tree_handler_.key_comparator_.attr_comparator();
    // 找出第一个不相等的属性，然后比较大小
    int result =  0 ;
                                              //这里添加condition仅仅是为了Operator的open阶段不要抛出段错误  
    for(auto i=0; i<attr_comparators.size(); i++) {    //这里其实还是有个bug的，对于一个空表进行查询where 只涉及多值索引其中部分字段，这里会报错
  ...
  if ( left_user_key.empty() || nullptr == left_user_key[0] ) {  //前者为单值索引，后者为多值
    ...
    if(left_user_key.size() != tree_handler_.file_header_.attr_num) {   //检查左侧key数
      LOG_WARN("the number of left user key is not equal to the number of attributes");
      return RC::INVALID_ARGUMENT;
    }

    std::vector<const char *> fixed_left_keys;  //这里初始化结束后 fixed_left_keys[0] == 0x0，一会儿make_key就抛出assert
    char * fixed_left_key;
    for(auto i=0; i<left_user_key.size(); i++) {
      fixed_left_keys.emplace_back(const_cast<char *>(left_user_key[i]));
      if (tree_handler_.file_header_.attr_types[i] == CHARS) {
        bool should_inclusive_after_fix = false;
        fixed_left_key = const_cast<char *>(fixed_left_keys[i]);        // 为什么  这里不能直接  &(const_cast<char *> fixed_left_keys[i])?
        rc = fix_user_key(left_user_key[i], left_len[i], true /*greater*/, &fixed_left_key, &should_inclusive_after_fix);
        fixed_left_keys[i]=fixed_left_key;
        if (rc != RC::SUCCESS) {
    ...
    MemPoolItem::unique_ptr left_pkey;  //key值有问题
    if (left_inclusive) {
    ...
    for(auto i=0; i<fixed_left_keys.size(); i++) {
    ...
    rc = tree_handler_.find_leaf(latch_memo_, BplusTreeOperationType::READ, left_key, current_frame_);  //找到叶子结点
    if (rc == RC::EMPTY) {
      rc = RC::SUCCESS;
    ...
    // lookup 返回的是-适合插入的位置，还需要判断一下是否在合适的边界范围内
    tree_handler_.free_key(const_cast<char *>(left_key));
  ...
  if ( right_user_key.empty() || nullptr == right_user_key[0]) {
    ...
    right_key_ = nullptr;
    if(right_user_key.size() != tree_handler_.file_header_.attr_num) {  //检查右侧key数
      LOG_WARN("the number of right user key is not equal to the number of attributes");
      return RC::INVALID_ARGUMENT;
    }

    std::vector<const char *> fixed_right_keys;
    //fixed_right_keys.resize(right_user_key.size());
    char *fixed_right_key;
    for(auto i=0; i<right_user_key.size(); i++) {
      fixed_right_keys.emplace_back(const_cast<char *>(right_user_key[i]));
      if (tree_handler_.file_header_.attr_types[i] == CHARS) {      
        bool should_inclusive_after_fix = false;
        fixed_right_key = const_cast<char *>(fixed_right_keys[i]);
        rc = fix_user_key(right_user_key[i], right_len[i], false /*want_greater*/, &fixed_right_key, &should_inclusive_after_fix);
        fixed_right_keys[i]=fixed_right_key;
        if (rc != RC::SUCCESS) {
    ...
    for(auto i=0; i<fixed_right_keys.size(); i++) {
    ...
    //const char *right_key = (const char *)right_key_.get();

    /*rc = tree_handler_.find_leaf(latch_memo_, BplusTreeOperationType::READ, right_key, current_frame_);
    if(rc == RC::EMPTY) {
      rc = RC::SUCCESS;
      current_frame_ = nullptr;
      return rc;
    } else if (rc != RC::SUCCESS) {
      LOG_WARN("failed to find right page. rc=%s", strrc(rc));
      return rc;
    }*/

    /*LeafIndexNodeHandler right_node(tree_handler_.file_header_, current_frame_);
    int right_index = right_node.lookup(tree_handler_.key_comparator_, right_key);
    tree_handler_.free_key(const_cast<char *>(right_key));
    // lookup 返回的是适合插入的位置，还需要判断一下是否在合适的边界范围内
    if(right_index > 0) {  // 超出了当前页，就需要向后移动一个位置
      const PageNum next_page_num = right_node.next_page();
      if(next_page_num == BP_INVALID_PAGE_NUM) {  // 这里已经是最后一页，说明当前扫描，没有数据
        latch_memo_.release();
        current_frame_ = nullptr;
        return RC::SUCCESS;
    ...
    const char *user_key, int key_len, bool want_greater, char **fixed_key, bool *should_inclusive)
{
  ...
  //assert(tree_handler_.file_header_.attr_num == 1); // 目前只支持单属性的索引
  //assert(tree_handler_.file_header_.attr_types[0] == CHARS);
  int tmp1 = strlen(user_key);
  int tmp2 = static_cast<size_t>(key_len);
