...
#define MAX_INDEX_COL_NUM 20  //最大索引数 20
/**
 * @brief B+树的实现
 * @defgroup BPlusTree
 */
...
{
public:
  void init(const std::vector<AttrType> &types, const std::vector<int> &lens)
  {
    assert(types.size() == lens.size());
    attr_comparators_.resize(types.size());   //这句话至关重要，没有初始化这个vector的话是要抛出段异常的
    for (int i = 0; i < types.size(); i++) {
      attr_comparators_[i].init(types[i], lens[i]);
    }
  }
  
  void init(const AttrType type, const int length)
  {
  ...
  int operator()(const char *v1, const char *v2) const
  {
    int result;
    int offset_ = 0;
    for (AttrComparator it: attr_comparators_) {
      result = it(v1 + offset_, v2 + offset_);
      if (result != 0) {
        return result;
      } 
      offset_ += it.attr_length();
    }
    if (result != 0) {
      return result;
    }

    //如果相等，进行RID的比较，此时的offset不用计算已经给出
    const RID *rid1 = (const RID *)(v1 + offset_);
    const RID *rid2 = (const RID *)(v2 + offset_);
...
public:
  ...
  std::string operator()(const char *v) const
  {
    ...
    for (int i = 0; i < attr_printers_.size(); ++i) {
      ss << attr_printers_[i](v + offset_) << ",";
      offset_ += attr_printers_[i].attr_length();
    }
    ss << "},";

    const RID *rid = (const RID *)(v + offset_);
...
struct IndexFileHeader 
{
  ...
  int32_t attr_num;           ///< 键值的个数 
  int32_t attr_length;        ///< 键值的长度
  int32_t attr_lengths[MAX_INDEX_COL_NUM];        ///< 多个键值的长度
  int32_t key_length;         ///< attr length + sizeof(RID)
  AttrType attr_type;         ///< 键值的类型
  AttrType attr_types[MAX_INDEX_COL_NUM];         ///< 多个键值的类型
...
public:
  ...
  RC insert_entry(std::vector<const char *>user_key, const RID *rid);  //考虑过user_key拿index_col一个一个加起来，但是const * 加完可能会很长很长，所以还是用vector吧
  ...
  //int key_size(int current) const;
...
private:
  common::MemPoolItem::unique_ptr make_key(std::vector<const char *> user_key, const RID &rid);
...
public:
   ...
   */
  RC open(const std::vector<const char *>left_user_key,const std::vector<int> left_len, bool left_inclusive, 
...
private:
  ...
  common::MemPoolItem::unique_ptr right_key_;  //表示当前迭代器所在的叶子节点的右兄弟节点的最小关键字，用于优化迭代器的性能。
  int iter_index_ = -1;    //迭代器所在位置在叶子节点中的索引
