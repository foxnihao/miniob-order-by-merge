...
public:
  ...
  virtual IndexScanner *create_scanner(std::vector<const char*>left_keys, std::vector<int> left_lens, bool left_inclusive,
                                       std::vector<const char*>right_keys, std::vector<int> right_lens, bool right_inclusive) = 0;
  /**
   * @brief 同步索引数据到磁盘
   * 
   */
...
protected:
  ...
  std::vector<FieldMeta> field_meta_;  ///< 多字段索引的字段元数据
};
