...
public:
  ...
  static RC create(Session *session, Db *db, const CreateTableSqlNode &create_table, Stmt *&stmt);

  const Value *subquery_result() const { return subquery_result_; }

private:
  std::string table_name_;
  std::vector<AttrInfoSqlNode> attr_infos_;
  Value *subquery_result_ = nullptr; // value list of value list
};