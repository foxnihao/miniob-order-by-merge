...
#include "sql/operator/table_alias_physical_operator.h"
...
RC PhysicalPlanGenerator::create(LogicalOperator &logical_operator, unique_ptr<PhysicalOperator> &oper)
{
  ...
  switch (logical_operator.type()) {
    ...
    case LogicalOperatorType::TABLE_ALIAS: {
      return create_plan(static_cast<TableAliasLogicalOperator &>(logical_operator), oper);
    } break;
...
RC PhysicalPlanGenerator::create_plan(ProjectLogicalOperator &project_oper, unique_ptr<PhysicalOperator> &oper)
{
  ...
  ProjectPhysicalOperator *project_operator =
      new ProjectPhysicalOperator(std::move(project_oper.expressions()), project_oper.outer_query_tuple);
...
RC PhysicalPlanGenerator::create_plan(AggreLogicalOperator &aggre_oper, std::unique_ptr<PhysicalOperator> &oper)
{
  ...
  AggrePhysicalOperator *aggre_physical_oper = new AggrePhysicalOperator(aggre_oper.groups());
  if (child_phy_oper) {
    aggre_physical_oper->add_child(std::move(child_phy_oper));
  }

  oper = unique_ptr<PhysicalOperator>(aggre_physical_oper);
...
{
  const std::vector<OrderSqlNode> &orders = orderby_oper.orders();
  vector<unique_ptr<LogicalOperator>> &child_opers = orderby_oper.children();
  unique_ptr<PhysicalOperator> child_physical_oper;  RC rc = RC::SUCCESS;
  if (!child_opers.empty()) {
    LogicalOperator *child_oper = child_opers.front().get();
  ...
  OrderByPhysicalOperator *orderby_physical_oper = new OrderByPhysicalOperator(orders);

  if (child_physical_oper) {
    orderby_physical_oper->add_child(std::move(child_physical_oper));
  }

  oper = unique_ptr<PhysicalOperator>(orderby_physical_oper);

  LOG_TRACE("create a orderby physical operator");
  return rc;
}

RC PhysicalPlanGenerator::create_plan(TableAliasLogicalOperator &table_alias_oper, unique_ptr<PhysicalOperator> &oper)
{
  vector<unique_ptr<LogicalOperator>> &child_opers = table_alias_oper.children();
  ...
  oper = unique_ptr<TableAliasPhysicalOperator>(new TableAliasPhysicalOperator(table_alias_oper.table_alias_map()));
  oper->add_child(std::move(child_phy_oper));
  return rc;
}
...
RC PhysicalPlanGenerator::create_plan(UpdateLogicalOperator &update_oper, unique_ptr<PhysicalOperator> &oper)
{
  ...
  oper = unique_ptr<PhysicalOperator>(new UpdatePhysicalOperator(update_oper.table(), update_oper.update_items()));
...
RC PhysicalPlanGenerator::create_plan(JoinLogicalOperator &join_oper, unique_ptr<PhysicalOperator> &oper)
{
  ...
  unique_ptr<PhysicalOperator> join_physical_oper(new NestedLoopJoinPhysicalOperator(join_oper.condition));
