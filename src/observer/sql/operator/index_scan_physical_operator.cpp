...
RC IndexScanPhysicalOperator::open(Trx *trx)
{
  ...
  auto index_meta = index_->field_meta();
  std::vector<const char*> left_keys;
  std::vector<int> lens;
  const std::vector<FieldMeta>* field_metas_ptr = table_->table_meta().field_metas();  // 调用 field_metas() 函数获取指针
  ...
  IndexScanner *index_scanner = index_->create_scanner(
    std::vector<const char*>{left_value_.data()}, 
...
RC IndexScanPhysicalOperator::filter(RowTuple &tuple, bool &result)
{
  ...
  for (std::unique_ptr<Expression> &expr : predicates_) {
    ...
    bool tmp_result = value.is_null() ? false : value.get_boolean();
    if (!tmp_result) {
      result = false;
      return rc;
    }
