...
RC PredicatePhysicalOperator::next()
{
  ...
  while (RC::SUCCESS == (rc = oper->next())) {
    ...
    if (rc != RC::SUCCESS) {
      LOG_WARN("predicate operator cannot get value from tuple");
      return rc;
    }

    if (value.attr_type() != BOOLEANS) {
      Value converted_value;
      rc = value.convert_into(BOOLEANS, converted_value);
      if (rc != RC::SUCCESS) {
        LOG_WARN("predicate expression got a value inconvertible to BOOLEAN");
        return rc;
      }
      value.set_value(converted_value);
    }
